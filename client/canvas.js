...
const Canvas = (function() {
    ...
    // Client only maintains strokes for rendering
    ...
    function handleMouseDown(event) {
        ...
        // Start a new stroke object for local drawing responsiveness
        // Will be sent to server on mouseup
        currentStroke = {
        ...
        // Draw the initial point locally for immediate feedback
    ...
    function handleMouseUp(event) {
        ...
        // Return completed stroke for sending to server
        // Server will handle adding it to history and broadcasting
    ...
    // Draw a single stroke (for stroke:add from server)
    function drawStroke(stroke) {
        if (!stroke || stroke.points.length === 0) {
            return;
        }
        
        // Check if stroke already exists (avoid duplicate drawing)
        const exists = strokes.some(s => s.id === stroke.id);
        if (exists) {
            return;
        }
    ...
    // Set strokes from server and redraw canvas (for canvas:reset)
    function setStrokesFromServer(serverStrokes) {
        // Replace local strokes with server strokes
        strokes = serverStrokes.map(s => ({ ...s }));
        
        // Clear and redraw canvas from stroke history
        redrawAllStrokes();
    }
